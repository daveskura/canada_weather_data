...
SELECT segment,postalcode,stationid,
    distance + (rnk * 0.0000000001) as distance 
FROM (
    SELECT *
        ,RANK() OVER (PARTITION BY segment,postalcode,distance ORDER BY stationid) as rnk
    FROM (
        SELECT segment,PC.postalcode,stationid,
...
CREATE INDEX ON canweather.distances(segment,stationid,postalcode);

SELECT COUNT(*)
FROM canweather.distances;

SELECT segment,postalcode,distance,count(*)
FROM canweather.distances
WHERE segment=1 -- and postalcode = 'B2G2T5' -- and stationid in (6463,6500)
GROUP BY segment,postalcode,distance
HAVING COUNT(*) > 1;
